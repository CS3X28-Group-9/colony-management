...
def resolve_fk_instance(fk_field: ForeignKey, raw_value: Any, project=None):
    ...
    if target_model is Mouse:
        tube_field = _get_model_field(Mouse, "tube_number")
        if tube_field is not None:
            ...
            return queryset.filter(tube_number=tube_value).first()

        pk_name = _target_pk_name(Mouse)
        pk_field = _get_model_field(Mouse, pk_name)
        pk_value = _coerce_for_field(pk_field, raw_value)
        if pk_value is None:
            return None
        return Mouse.objects.filter(**{pk_name: pk_value}).first()
...
def link_self_foreign_keys(
    ...
    field_by_name: Dict[str, ForeignKey],
    project,
    errors: List[str],
) -> None:
